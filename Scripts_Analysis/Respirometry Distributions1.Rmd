...
ggplot(data=dataBEUZmass,aes(x=day,y=co2_min_mass,color=treat,group=strain))+
  stat_summary(fun.y=mean, geom="line")+
  stat_summary(fun.data=mean_se, geom="errorbar", width=0.05) +
  theme_classic()+
  scale_x_continuous(breaks=pretty(dataBEUZmass$day, n=30))+
...
ggplot(data=dataBEUZ16,aes(x=day,y=co2_min_mass,color=strain))+
  ...
  theme(panel.grid.major = element_line(size=0.5),panel.grid.minor = element_line(size=0))+
  scale_fill_manual(values=c(UZ="lightblue",BE="lightred"))+
...
ggplot(data=massdiff,aes(x=day,y=co2_min_mass,group=term,color=term))+
  ...
  scale_x_continuous(breaks=pretty(massdiff$day, n=14))+
  theme(panel.grid.major = element_line(size = 0.5),panel.grid.minor = element_line(size=0))+
...
modco2peakm=aov(co2_min_mass~term,data=massdiff)
summary(modco2peakm)
