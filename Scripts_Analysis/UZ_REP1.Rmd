...
dataUZ=data
dataUZ=subset(dataUZ, strain=="UZ")
dataUZ=subset(dataUZ, stage=="L")
dataUZ=subset(dataUZ, treat=="1"|treat=="2")
par(mfrow=c(1,1))
...
qplot(treat, co2_hourly_mass, data=dataUZ, geom="boxplot")
rmod1=lmer(co2_hourly_mass~treat*day+(1|day),data=dataUZ) #different intercepts
#rmod2=lmer(co2_hourly_mass~treat*day+(1|day)+(1|day/syringe),data=dataUZ) #random factor nested within a level
rmod3=lmer(co2_hourly_mass~treat*day+(day|syringe),data=dataUZ) #different slopes and intercepts
...
rmoda=lmer(co2_hourly_mass~(1|day),data=dataUZ) #model w/o independent var
rmodb=lmer(co2_hourly_mass~treat+(1|day),data=dataUZ) # 1 independent var
rmodc=lmer(co2_hourly_mass~treat+day+(1|day),data=dataUZ) # 2 independent vars w/o intrx
rmodd=lmer(co2_hourly_mass~treat*day+(1|day),data=dataUZ) # 2 independent vars w/ intrx
anova(rmoda,rmodb)#b is UZtter
anova(rmoda,rmodc)#c is UZtter
anova(rmoda,rmodd)#d is UZtter
anova(rmodb,rmodc)#c is UZtter
anova(rmodb,rmodd)#d is UZtter
anova(rmodc,rmodd)#d is UZtter
summary(rmod1) #rmod1/rmodd wins
````
...
tendays=data1to10
gg <- ggplot(data=tendays, aes(x = day, y = co2_hourly_mass)) +
...
##Regression comparison UZtween multilevel, pooled and unpooled data
ml.model <- lmer(co2_hourly_mass~treat*day+(1|day),data=tendays)
